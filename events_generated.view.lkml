...
view: events_generated {
  ...
  dimension: events_app_update.previous_app_version {
    type: string
    sql:
      CASE WHEN ${name} = 'app_update' THEN
  ...
  dimension: events_collection_unlocked.id {
    type: string
    sql:
      CASE WHEN ${name} = 'collection_unlocked' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'id')
      END ;;
  ...
  dimension: events_facebook_account_linked.source {
    type: string
    sql:
      CASE WHEN ${name} = 'facebook_account_linked' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'source')
      END ;;
  ...
  dimension: events_facebook_login_dialog_accepted.source {
    type: string
    sql:
      CASE WHEN ${name} = 'facebook_login_dialog_accepted' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'source')
      END ;;
  ...
  dimension: events_facebook_login_dialog_canceled.source {
    type: string
    sql:
      CASE WHEN ${name} = 'facebook_login_dialog_canceled' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'source')
      END ;;
  ...
  dimension: events_facebook_login_dialog_popped.source {
    type: string
    sql:
      CASE WHEN ${name} = 'facebook_login_dialog_popped' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'source')
      END ;;
  ...
  dimension: events_facebook_session_changed.hasError {
    type: number
    sql:
      CASE WHEN ${name} = 'facebook_session_changed' THEN
  ...
  dimension: events_first_open.firebase_conversion {
    type: number
    sql:
      CASE WHEN ${name} = 'first_open' THEN
        (SELECT value.int_value
        FROM UNNEST(${params})
  ...
  dimension: events_graph_subscribe_on_client.facebook {
    type: number
    sql:
      CASE WHEN ${name} = 'graph_subscribe_on_client' THEN
        (SELECT value.int_value
        FROM UNNEST(${params})
        WHERE key = 'facebook')
      END ;;
  ...
  dimension: events_in_app_purchase.currency {
    type: string
    sql:
      CASE WHEN ${name} = 'in_app_purchase' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'currency')
      END ;;
  ...
  dimension: events_item_purchased.chip_cost {
    type: number
    sql:
      CASE WHEN ${name} = 'item_purchased' THEN
        (SELECT value.int_value
        FROM UNNEST(${params})
        WHERE key = 'chip_cost')
      END ;;
  ...
  dimension: events_level_lock_reached.updated_prompted {
    type: number
    sql:
      CASE WHEN ${name} = 'level_lock_reached' THEN
        (SELECT value.int_value
        FROM UNNEST(${params})
  ...
  dimension: events_local_notif_clicked.type {
    type: string
    sql:
      CASE WHEN ${name} = 'local_notif_clicked' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'type')
      END ;;
  ...
  dimension: events_out_of_chips_dialog_popped.level {
    type: number
    sql:
      CASE WHEN ${name} = 'out_of_chips_dialog_popped' THEN
        (SELECT value.int_value
        FROM UNNEST(${params})
        WHERE key = 'level')
      END ;;
  ...
  dimension: events_push_notif_clicked.type {
    type: string
    sql:
      CASE WHEN ${name} = 'push_notif_clicked' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'type')
      END ;;
  ...
  dimension: events_push_notif_sent.type {
    type: string
    sql:
      CASE WHEN ${name} = 'push_notif_sent' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'type')
      END ;;
  ...
  dimension: events_round_completed.bingos_claimed {
    type: number
    sql:
      CASE WHEN ${name} = 'round_completed' THEN
        (SELECT value.int_value
        FROM UNNEST(${params})
  ...
  dimension: events_select_content.content_type {
    type: string
    sql:
      CASE WHEN ${name} = 'select_content' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
  ...
  dimension: events_server_error_fatal.code {
    type: number
    sql:
      CASE WHEN ${name} = 'server_error_fatal' THEN
        (SELECT value.int_value
        FROM UNNEST(${params})
        WHERE key = 'code')
      END ;;
  ...
  dimension: events_server_error_single.code {
    type: number
    sql:
      CASE WHEN ${name} = 'server_error_single' THEN
        (SELECT value.int_value
        FROM UNNEST(${params})
        WHERE key = 'code')
      END ;;
  ...
  dimension: events_spend_virtual_currency.item_name {
    type: string
    sql:
      CASE WHEN ${name} = 'spend_virtual_currency' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'item_name')
      END ;;
  ...
  dimension: events_store_purchase_saved.success {
    type: number
    sql:
      CASE WHEN ${name} = 'store_purchase_saved' THEN
        (SELECT value.int_value
        FROM UNNEST(${params})
        WHERE key = 'success')
      END ;;
  ...
  dimension: events_store_purchase_started.product_id {
    type: string
    sql:
      CASE WHEN ${name} = 'store_purchase_started' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'product_id')
      END ;;
  ...
  dimension: events_unlock_achievement.name {
    type: string
    sql:
      CASE WHEN ${name} = 'unlock_achievement' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
        WHERE key = 'name')
      END ;;
  ...
  dimension: events_use_time_extender.num_extends {
    type: number
    sql:
      CASE WHEN ${name} = 'use_time_extender' THEN
        (SELECT value.int_value
        FROM UNNEST(${params})
  ...
  dimension: events_user_engagement.engagement_time_msec {
    type: number
    sql:
      CASE WHEN ${name} = 'user_engagement' THEN
        (SELECT value.int_value
        FROM UNNEST(${params})
  ...
  dimension: events_user_logged_in.with_facebook {
    type: string
    sql:
      CASE WHEN ${name} = 'user_logged_in' THEN
        (SELECT value.string_value
        FROM UNNEST(${params})
